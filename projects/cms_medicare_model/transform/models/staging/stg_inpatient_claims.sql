...
select
    ...
    cast(strptime(cast(CLM_ADMSN_DT as string), '%Y%m%d') as date) as admission_date,
    ADMTNG_ICD9_DGNS_CD as admitting_diagnosis_code,
    cast(NCH_BENE_IP_DDCTBL_AMT as double) as beneficiary_deductible_amount,
    cast(NCH_BENE_PTA_COINSRNC_LBLTY_AM as double) as beneficiary_coinsurance_amount,
    cast(NCH_BENE_BLOOD_DDCTBL_LBLTY_AM as double) as beneficiary_blood_deductible_amount,
    CLM_UTLZTN_DAY_CNT as utilization_day_count,
    cast(strptime(cast(NCH_BENE_DSCHRG_DT as string), '%Y%m%d') as date) as discharge_date,
